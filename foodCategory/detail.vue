	...
	export default {
		...
		async onShow(options) {
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('foodCategory', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','foodCategory');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','foodCategory');
			...
			async init(type=1){
				...
				let res = await this.$api.info('foodCategory', this.id);
				let reg=new RegExp('http://localhost:8080/healthy_diet/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('foodCategory',this.detail)
				}
			},
