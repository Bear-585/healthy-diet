			...
			<view :style='{"padding":"0 0 220rpx 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 24rpx","background":"#F8E2BE","display":"flex","height":"auto"}'>
					<view  :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' class="search-form round">
						...
						<input placeholder-style="color: #999" :style='{"border":"2rpx solid #FAC696","padding":"12rpx 20rpx 12rpx 80rpx","boxShadow":"none","color":"#7A531A","borderRadius":"10rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.foodCategory" type="text" placeholder="食品分类" ></input>
					</view>
					...
					<view class="list" :style='{"width":"calc(100% - 200rpx)","padding":"0 20rpx","margin":"0","flex":"1","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"8rpx","margin":"0 0 20rpx","borderRadius":"20rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view :style='{"width":"calc(100% - 0px)","padding":"0","margin":"20rpx 0 0","justifyContent":"center","display":"flex","height":"auto"}'>
								<view :style='{"border":"2rpx solid #F49843","margin":"0 5% 10rpx 0","borderRadius":"8rpx","background":"#FEF4E4","display":"flex","width":"40%","justifyContent":"center"}' v-if="(userid && isAuth('foodCategory','修改')) || (!userid && isAuthFront('foodCategory','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"border":"2rpx solid #999898","margin":"0 0 10rpx 5%","borderRadius":"8rpx","background":"#EDEDED","display":"flex","width":"40%","justifyContent":"center"}' v-if="(userid && isAuth('foodCategory','删除')) || (!userid && isAuthFront('foodCategory','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"calc(100% - 0px)","padding":"0","margin":"20rpx 0 0","justifyContent":"center","display":"flex","height":"auto"}'>
								<view :style='{"border":"2rpx solid #F49843","margin":"0 5% 10rpx 0","borderRadius":"8rpx","background":"#FEF4E4","display":"flex","width":"40%","justifyContent":"center"}' v-if="(userid && isAuth('foodCategory','修改')) || (!userid && isAuthFront('foodCategory','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"26rpx","lineHeight":"50rpx","color":"#F49843","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"26rpx","lineHeight":"50rpx","color":"#F49843","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"border":"2rpx solid #999898","margin":"0 0 10rpx 5%","borderRadius":"8rpx","background":"#EDEDED","display":"flex","width":"40%","justifyContent":"center"}' v-if="(userid && isAuth('foodCategory','删除')) || (!userid && isAuthFront('foodCategory','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"26rpx","lineHeight":"50rpx","color":"#999898","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"26rpx","lineHeight":"50rpx","color":"#999898","display":"inline-block"}'>删除</text>
								</view>
							</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.foodCategory){
					params['foodCategory'] = '%' + this.searchForm.foodCategory + '%'
				}
				let user = uni.getStorageSync("appUserid")?JSON.parse(uni.getStorageSync('userSession')):{}
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`foodCategory`, params);
                } else {
                    res = await this.$api.list(`foodCategory`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await that.$api.del('foodCategory', JSON.stringify([id]));
							that.$utils.msg('删除成功');
							that.hasNext = true
							// 重置分页参数页数为1
							that.search()
			...
			async search(){
				...
				if(this.searchForm.foodCategory){
					searchForm['foodCategory'] = '%' + this.searchForm.foodCategory + '%'
				}
				let res = {};
				if(this.userid) {
					res = await this.$api.page(`foodCategory`, searchForm);
				} else {
					res = await this.$api.list(`foodCategory`, searchForm);
				}
				// 如果是第一页数据置空
